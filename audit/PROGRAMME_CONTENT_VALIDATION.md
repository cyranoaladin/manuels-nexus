# Rapport de Conformite Programme et Exactitude des Contenus (LOT 3)

- **Atomes officiels obligatoires** : 596
- **Atomes cartographies** : 596
- **Atomes officiels non couverts** : `0`
- **Fausses couvertures** : `44`
- **Contenus hors programme non etiquetes** : `0`
- **Divergences de reponses independantes** : `0`
- **Validations formelles executees** : 437
- **Validations passees** : 414
- **Objets de revue manuelle (théorique/conceptuel)** : 23
- **Défauts concrets trouvés** : `0`
- **Contenus non formalisables sans défaut** : 19

## Analyse adversariale des revues manuelles
- Objets en verdict `manual_review` : 23
- Sans défaut concret : 19
- Défauts concrets encore ouverts : `0`
- Objets non relus (aucune disposition enregistrée) : `0`
