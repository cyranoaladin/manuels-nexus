...
2. **Paramétrage exécution Python (verify_python)** quand les valeurs sont numériques : déclarer `parametres_sympy` dans META (ex. `{"u0": {"domaine": "entier", "min": 1, "max": 20}, "q": {"domaine": "rationnel", "contrainte": "q>0, q!=1"}}`) et écrire l'énoncé avec les valeurs instanciées par défaut.
...
## Exigences NSI supplémentaires
- Tout exercice à code : bloc % BEGIN-VERIFY (asserts sur la fonction du corrigé) ; tout
  exercice « que fait ce programme / donner la sortie » : bloc % BEGIN-TRACE / % EXPECTED
  (la sortie affichée est produite par exécution, jamais écrite de tête).
- Typologie à couvrir dans chaque matrice : écrire une fonction (spec donnée), lire/tracer
  un code, corriger un code bogué (le bogue = une erreur fréquente documentée), compléter
  un code, QCM de prédiction, exercice papier sans machine (représentation mémoire).
- ◆◆◆ NSI = conception : choisir la structure de données, spécifier puis implémenter,
  analyser un coût, généraliser une fonction.
- Code élève ruff clean ; en Terminale, annotations de type dans les corrigés.
