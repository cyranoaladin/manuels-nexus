...
Remplacer la borne incorrecte par une preuve explicite : \`j\` peut atteindre
\`-1\`, tandis que le variant entier \`j + 1\` reste positif avant chaque tour
execute et decroit strictement. La boucle termine donc apres un nombre fini de
decalages.
...
- la presence du variant \`j + 1\` et de la borne \`-1\` dans la preuve ;
- la presence de « premier element » et de « nombre d'elements du tableau »
  dans le bloc Q2, ainsi que l'absence de « dernier element » dans ce bloc.
...
restent tracables dans l'historique Git.
