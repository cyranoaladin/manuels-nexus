...
def test_the_two_editorial_decisions_are_escalated_not_guessed() -> None:
    ledger = _json(LEDGER)
    pending = ledger["deltas_requiring_human_editorial_decision"]
    assert len(pending) == 2
    for entry in pending:
        assert entry["decision_required"]
        assert entry["authority_cannot_settle"]
