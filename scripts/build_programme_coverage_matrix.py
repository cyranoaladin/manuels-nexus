...
def load_contracts(config: dict) -> tuple[dict[str, dict], list[dict]]:
    ...
    for contract_path in sorted((manual_root / "chapitres").glob(f"{config['chapter_prefix']}*/contrat.yaml")):
        ...
        for capacity in contract.get("capacites", []):
            facet = capacity.get("sans_alias_officiel")
            data = {
                "chapter": contract["chapitre"],
                "contract_capacity": capacity["code"],
                "ref_capacite": capacity.get("ref_capacite"),
                "student_wording": capacity.get("libelle_eleve", ""),
                "contract_path": str(contract_path.relative_to(ROOT)),
            }
            if facet:
                # Facette pedagogique d'une capacite officielle deja portee par un
                # autre code local. Lui attribuer la reference officielle la
                # crediterait une seconde fois ; la taire perdrait la preuve que
                # les objets qui la portent travaillent cette capacite. Elle est
                # donc declaree a part, rattachee a ce qu'elle decoupe.
                data["facet_of"] = facet["facette_de"]
                data["carried_by"] = facet["porte_par"]
                extra.append(data)
            elif capacity["ref_capacite"] in NON_CAPACITY_CONTRACT_REFS:
                extra.append(data)
            else:
...
def meta_capacity_refs(chapter_path: Path, contract: dict) -> dict[str, dict[str, set[str]]]:
    # Une facette est resolue vers la capacite officielle dont elle est une
    # facette : un objet qui la porte constitue bien une preuve pour celle-ci.
    code_to_ref = {
        capacity["code"]: (
            capacity["ref_capacite"]
            if "ref_capacite" in capacity
            else capacity["sans_alias_officiel"]["facette_de"]
        )
        for capacity in contract.get("capacites", [])
    }
...
def build_payload() -> dict:
    ...
    atoms = []
    #: Facettes pedagogiques : elles decoupent une capacite officielle deja
    #: portee ailleurs. Elles ne sont donc PAS des atomes officiels, et sont
    #: declarees a part plutot que tues.
    facets: list[dict] = []
    ...
    for manual, config in MANUAL_CONFIG.items():
        ...
        for extra in extras:
            if extra.get("facet_of"):
                facets.append({"manual": manual, **extra})
                continue
    ...
    return {
        ...
        "atoms": atoms,
        "contract_facets_without_official_alias": facets,
    }
