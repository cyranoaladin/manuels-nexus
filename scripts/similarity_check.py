...
from common import ROOT, db_connect, write_json
...
def check_object(tex_path: Path, chap_dir: Path) -> str:
    ...
    with db_connect() as conn, conn.cursor() as cur:
        ...
        cur.execute("SELECT id, content_md, tier, usage_policy FROM chunks WHERE theme = %s OR theme IS NULL",
                    (theme,))
        for cid, content, tier, policy in cur:
            src = ngrams(content)
            if not src:
                continue
            score = len(obj_ngrams & src) / max(1, len(obj_ngrams | src) - len(src - obj_ngrams))
