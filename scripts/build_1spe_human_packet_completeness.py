...
def chapter_verdicts() -> list[dict[str, Any]]:
    """L'état des vingt verdicts de chapitre -- la seule unité de gouvernance.

    Les files de relecture comptent des milliers d'« items » : 2 325 objets,
    167 questions de QCM, et ainsi de suite. Ce ne sont pas des signatures.
    Ce sont des points d'ATTENTION à l'intérieur d'un chapitre, et la revue
    porte sur le chapitre entier. Les présenter comme une charge de travail
    reviendrait à annoncer des centaines de décisions là où le contrat en
    demande vingt.
    """

    rows: list[dict[str, Any]] = []
    for directory in sorted(REVIEWS.glob("1SPE-*")):
        ...
        for key in ("review_a", "review_b"):
            ...
            rows.append(
                {
                    "chapter": state.get("chapter_id", directory.name),
...
def build() -> dict[str, Any]:
    ...
    verdicts = chapter_verdicts()
    rendered = [row for row in verdicts if row["verdict"] is not None]
    return {
        ...
        "the_unit_of_governance_is_the_chapter_verdict": (
            "Dix chapitres, deux rôles : vingt verdicts, et rien de plus. Les "
            "milliers d'items que portent les files de relecture sont des "
            "points d'attention À L'INTÉRIEUR d'un chapitre ; les compter "
            "comme des signatures annoncerait des centaines de décisions là où "
            "le contrat en demande vingt."
        ),
        ...
        "summary": {
            ...
            "CHAPTER_VERDICTS_EXPECTED": len(verdicts),
            "CHAPTER_VERDICTS_RENDERED": len(rendered),
            "CHAPTER_VERDICTS_PENDING": len(verdicts) - len(rendered),
        },
    }
...
def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"> {payload['the_unit_of_governance_is_the_chapter_verdict']}",
        "",
