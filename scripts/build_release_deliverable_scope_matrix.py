...
def build() -> dict[str, Any]:
    ...
    for manual, spec in sorted(specs.items()):
        ...
        for variant in sorted(spec.get("variants", {})):
            ...
            rows.append({
                ...
                "declared_assembly": bool(coverage.get("declared_variants")),
                "observed_build": bool(coverage.get("observed_variants")),
                **classification,
            })
    ...
    summary = {
        ...
        "REQUIRED_DELIVERABLES_WITHOUT_BUILD": sum(
            1 for r in required if not r["observed_build"]
        ),
...
def render_md(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- `REQUIRED_DELIVERABLES_WITHOUT_BUILD` : `{s['REQUIRED_DELIVERABLES_WITHOUT_BUILD']}`",
        "",
    ...
    for row in payload["deliverables"]:
        lines.append(
            ...
            f"{'oui' if row['observed_build'] else '**non**'} |"
        )
    lines.append("")
    return "\n".join(lines)
