---
title: "P00 - Trace - Diagnostic Python"
level: "premiere"
sequence_id: "P00"
document_type: "trace"
status: "needs_review"
...
# P00 - Trace écrite - Diagnostic Python
...
### Repère 1 - trace d’affectation
- À retenir : dresser une table instruction, expression calculée, nouvelle valeur.
- Exemple minimal : `x = 4 ; x = x + 3 ; print(x)` donne `7` affiché.
- Vigilance : contrôler « variable réaffectée deux fois ».
- Question flash : que devient la méthode si la donnée change légèrement ?
### Repère 2 - concaténation de chaînes
- À retenir : identifier le type chaîne puis appliquer `+` comme concaténation.
- Exemple minimal : `mot = "NS" ; mot = mot + "I"` donne `"NSI"`.
- Vigilance : contrôler « addition impossible entre chaîne et entier ».
- Question flash : que devient la méthode si la donnée change légèrement ?
### Repère 3 - test conditionnel
- À retenir : calculer le reste puis évaluer le booléen.
- Exemple minimal : `n = 6 ; n % 2 == 0` donne `True`.
- Vigilance : contrôler « reste nul ».
- Question flash : que devient la méthode si la donnée change légèrement ?
### Repère 4 - fonction courte
- À retenir : substituer l’argument et distinguer retour et affichage.
- Exemple minimal : `def f(a): return 2*a + 1` avec `a = 5` donne `11` retourné.
- Vigilance : contrôler « aucun affichage sans `print` ».
- Question flash : que devient la méthode si la donnée change légèrement ?
## Exercices numérotés
