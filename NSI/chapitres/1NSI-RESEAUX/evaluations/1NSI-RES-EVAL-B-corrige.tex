...
\begin{itemize}
  \item après \lstinline{traiter_commande("AUTO")}, \lstinline{ventilation_active(1000)}
  ...
  \item après \lstinline{traiter_commande("ON")}, \lstinline{ventilation_active(500)} vaut
  \lstinline{True} : le forcage l'emporte sur un taux bas ;
  \item après \lstinline{traiter_commande("OFF")}, \lstinline{ventilation_active(1000)}
  vaut \lstinline{False} : l'interdiction l'emporte sur un taux eleve.
\end{itemize}
On y ajoutera utilement le refus de \lstinline{traiter_commande("MAX")}, qui vérifie la
