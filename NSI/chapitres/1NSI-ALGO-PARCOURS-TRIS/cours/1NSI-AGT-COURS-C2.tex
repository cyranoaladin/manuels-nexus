% META: {"id": "1NSI-AGT-COURS-C2", "chapitre": "1NSI-ALGO-PARCOURS-TRIS", "type_objet": "cours", "capacites": ["P-ALGO-02A", "P-ALGO-02C"], "mode_creation": "ex_nihilo", "sources_inspiration": [], "status": "verified"}
...
\propriete[Terminaison et coût]{
  ...
  ($\max(n-1,0)$ fois pour un tableau de taille $n$) : elle termine nécessairement. Pour
  $n\leqslant1$, aucune itération n'est nécessaire : ces tableaux sont déjà triés. Dans la
  boucle \lstinline{while} interne, avant chaque tour exécuté, $j\geqslant0$ ; le variant
  entier $j+1$ est donc \textbf{strictement positif} et décroît de $1$. Lorsque $j=-1$,
  la condition de la boucle devient fausse. Dans le \textbf{pire cas} (tableau trié en
  ordre décroissant), le coût est \textbf{quadratique} ($O(n^2)$) : chaque insertion peut
  nécessiter de décaler tous les éléments déjà triés.
}
...
\textbf{Initialisation.} Lorsque $n\geqslant2$, avant la première itération ($i=1$),
\lstinline{tableau[0:1]} est un tableau à un seul élément : il est trivialement trié.
...
\textbf{Terminaison.} Si $n\leqslant1$, la boucle ne s'exécute pas et le tableau entier
est déjà trié. Si $n\geqslant2$, la dernière itération a $i=n-1$ et l'invariant donne :
\lstinline{tableau[0:n]} (le tableau entier) est trié. L'algorithme est donc \textbf{correct}.
}
