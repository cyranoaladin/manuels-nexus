% META: {"id": "1NSI-AGT-COURS-C3", "chapitre": "1NSI-ALGO-PARCOURS-TRIS", "type_objet": "cours", "capacites": ["P-ALGO-02B", "P-ALGO-02D"], "mode_creation": "ex_nihilo", "sources_inspiration": [], "status": "verified"}
...
\propriete[Terminaison et coût]{
  Pour un tableau de taille $n$, la boucle externe s'exécute
  $\max(n-1,0)$ itérations. Si $n\leqslant1$, la boucle externe ne s'exécute pas
  et le tableau est déjà trié. Sinon, les deux boucles \lstinline{for} parcourent
  des intervalles finis, déterminés à l'avance : l'algorithme termine
  nécessairement. Pour $n$ variable, le coût est \textbf{quadratique}
  ($O(n^2)$) : contrairement au tri par insertion, la recherche du minimum
  restant parcourt toujours l'intégralité de la partie non triée, même si le
...
\textbf{Terminaison.} Si $n\leqslant1$, la boucle externe ne s'exécute pas et le
tableau est déjà trié. Si $n\geqslant2$, la dernière itération a $i=n-2$. Après
sa conservation, l'invariant donne que \lstinline{tableau[0:n-1]} est trié et que
son dernier élément est inférieur ou égal à l'unique élément de
\lstinline{tableau[n-1:n]} : le tableau entier est donc trié.
}
