...
\begin{itemize}
  \item champ de texte \lstinline{"recherche"} : événement de \textbf{saisie}, declenche a
  chaque frappe -- on filtre la liste affichee au fur et a mesure ;
  \item menu deroulant \lstinline{"tri"} : événement de \textbf{changement de selection} --
  on reordonne la liste selon le critere choisi ;
  \item bouton \lstinline{"valider"} : événement de \textbf{clic} -- on envoie la requete au
  serveur.
\end{itemize}
Sans fonction associee, le composant reste inerte : le HTML décrit l'apparence, il ne décrit
aucun traitement.

\textbf{2.} L'événement de saisie se declenche \textbf{plusieurs fois}, a chaque caractere,
et sert a un retour immediat sans quitter la page ; l'événement de validation se declenche
...
\begin{itemize}
  \item \textbf{Authentification} : le \textbf{client} affiche le formulaire et vérifie
  eventuellement que les champs sont remplis ; le \textbf{serveur} vérifie l'identifiant et
  le mot de passe, car lui seul detient les donnees de référence.
...
\textbf{3.} Le chiffrement est indispensable \textbf{des la première étape}, celle qui
transporte le mot de passe, et il doit ensuite être maintenu car le jeton de session
...
et le serveur ; il ne protege ni le poste de l'utilisateur, ni le serveur lui-même.
\end{corrige}
