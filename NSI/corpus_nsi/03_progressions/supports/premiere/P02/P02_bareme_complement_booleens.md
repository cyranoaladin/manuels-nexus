...
# P02 - Barème - Complément à deux et booléens
...
### Barème question 1
- 1 point : identifier correctement décodage signé et la donnée `11110110` sur 8 bits.
- 1 point : appliquer la méthode « lire le bit de signe puis soustraire `2^8` à la valeur naturelle ».
- 1 point : obtenir `-10`.
- 1 point : contrôler le cas limite « bit de poids fort à 1 » ou expliquer pourquoi il ne s’applique pas.
- Retrait possible : confusion avec l’erreur fréquente EF1 - Lire un mot binaire signé comme un entier naturel.
### Barème question 2
- 1 point : identifier correctement bornes sur n bits et la donnée `n = 4` bits signés.
- 1 point : appliquer la méthode « calculer `-2^(n-1)` et `2^(n-1)-1` ».
- 1 point : obtenir `[-8 ; 7]`.
- 1 point : contrôler le cas limite « asymétrie entre minimum et maximum » ou expliquer pourquoi il ne s’applique pas.
- Retrait possible : confusion avec l’erreur fréquente EF2 - Oublier de tester les bornes avant l’encodage.
### Barème question 3
- 1 point : identifier correctement encodage négatif et la donnée `-6` sur 8 bits.
- 1 point : appliquer la méthode « partir de 6, inverser les bits puis ajouter 1 ».
- 1 point : obtenir `11111010`.
- 1 point : contrôler le cas limite « retenue finale ignorée sur la largeur fixée » ou expliquer pourquoi il ne s’applique pas.
- Retrait possible : confusion avec l’erreur fréquente EF3 - Inverser les bits sans ajouter 1.
### Barème question 4
- 1 point : identifier correctement simplification booléenne et la donnée `(a and b) or (a and not b)`.
- 1 point : appliquer la méthode « dresser les quatre lignes de vérité ».
- 1 point : obtenir `a`.
- 1 point : contrôler le cas limite « un exemple ne prouve pas une identité » ou expliquer pourquoi il ne s’applique pas.
- Retrait possible : confusion avec l’erreur fréquente EF4 - Simplifier une expression booléenne avec un seul exemple.
## Exercices numérotés
