...
| 2026-07-16 | Collection | RÉSERVES F | **F.1** : test E2E réel (copie gabarits sans icons → compile → warning dans log → verify_pdf=1), 6/6 tests PDF. **F.2** : inspection exercices/QCM/TD (7 PNG : losanges, accents, encadrés PASS). **F.3** : `pdf_integrity.py` ajouté à `check_charte_sync.py` (7/7). **F.4** : workflows déplacés vers `.github/workflows/` racine (les sous-répertoires ne sont pas lus par GitHub Actions) ; `make test` 342+214, `check_charte_sync` 7/7. | MODE FICHIERS | [INTEGRITE] |
