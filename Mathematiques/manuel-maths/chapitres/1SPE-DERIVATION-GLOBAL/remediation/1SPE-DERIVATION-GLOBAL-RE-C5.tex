% META: {"id":"1SPE-DERIVATION-GLOBAL-RE-C5","chapitre":"1SPE-DERIVATION-GLOBAL","type_objet":"remediation","capacites_codes":["C5"],"status": "approved"}
...
% BEGIN-VERIFY
% from sympy import symbols, diff, solve, expand, Rational, simplify
% x = symbols('x')
% V = x * (10 - 2*x)**2
% Vexp = expand(V)
% assert simplify(Vexp - (4*x**3 - 40*x**2 + 100*x)) == 0
% Vp = diff(V, x)
% roots = solve(Vp, x)
% valid = [r for r in roots if 0 < r < 5]
% assert valid[0] == Rational(5, 3)
% Vmax = V.subs(x, Rational(5, 3))
% assert Vmax == Rational(2000, 27)
% END-VERIFY
...
\commentaireMarge{Calculer $\Delta$ puis $\sqrt{\Delta}$.}
