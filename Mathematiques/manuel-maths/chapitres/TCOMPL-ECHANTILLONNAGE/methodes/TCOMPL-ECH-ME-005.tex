...
\pasApas{%
  \begin{enumerate}
    \item Simuler UNE observation : \texttt{random.random() < p} renvoie
      \texttt{True} (succes) avec probabilite $p$.
...
\begin{flushleft}\ttfamily\small
  ...
  frequences = []\\
  for \_ in range(200):\quad\# N echantillons\\
  \hspace*{1.2em}succes = sum(1 for \_ in range(100)\\
  \hspace*{3.4em}if random.random() < 0.3)\\
  \hspace*{1.2em}frequences.append(succes / 100)\\
...
\pieges{%
  \begin{itemize}
    \item \textbf{Piege 1.} Ecrire \texttt{random.random() < p} avec $p$ en
