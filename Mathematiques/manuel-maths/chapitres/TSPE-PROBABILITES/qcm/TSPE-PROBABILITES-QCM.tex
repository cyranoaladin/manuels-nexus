...
\begin{itemize}
  \item \textbf{Q1}
  \begin{itemize}
    \item \textbf{A} — Cette expression donne la probabilité d'une configuration précise de $k$ succès ; elle oublie les $C(n,k)$ positions possibles des succès. \emph{Renvoi : C5.}
  ...
  \item \textbf{Q3}
  \begin{itemize}
    \item \textbf{A} — L'indépendance est requise pour additionner les variances, pas pour la linéarité de l'espérance de variables integrables.
  ...
  \item \textbf{Q4}
  \begin{itemize}
    \item \textbf{B} — Si $X=Y$, alors $V(X+Y)=V(2X)=4V(X)$, tandis que $V(X)+V(Y)=2V(X)$ : l'égalité n'est donc pas garantie.
  ...
  \begin{itemize}
    ...
    \item \textbf{C} — L'expression $n(1-p)$ oublie le facteur $p$ associe aux succès ; chaque indicatrice de Bernoulli a pour variance $p(1-p)$.
  ...
  \begin{itemize}
    ...
    \item \textbf{D} — Bienayme-Tchebychev concerne l'événement $|Y-\mu|\geq\delta$, pas la probabilité ponctuelle que $Y$ soit exactement égal a $\mu$.
  \end{itemize}
\end{itemize}
\fi
