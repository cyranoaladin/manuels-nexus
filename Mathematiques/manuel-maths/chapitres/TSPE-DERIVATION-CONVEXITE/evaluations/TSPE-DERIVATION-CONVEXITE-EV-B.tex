...
\textbf{Évaluation B -- Dérivation et convexité}
...
Soit $g(x) = x^3\,\mathrm{e}^{-x}$ définie sur $\mathbb{R}$.
\begin{enumerate}
...
\begin{enumerate}
  \item Soit $h(x) = \mathrm{e}^x - 2x - 1$. Calculer $h''(x)$ et en déduire la convexité de $h$.
  \item Déterminer le minimum de $h$ et preciser sa valeur.
  \item En utilisant la convexité, déterminer la position de la courbe de $\mathrm{e}^x$ par rapport a sa tangente en $\ln 2$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Déterminer l'équation de la tangente au point d'abscisse $3$ et preciser si la courbe la traverse.
\end{enumerate}
