...
\begin{itemize}
  \item \textbf{Q1}
  \begin{itemize}
    \item \textbf{B} — L'eleve a soustrait les deux composantes de $\vec u$ et calcule $3-(-2)=5$, au lieu de former les produits de composantes $3\times1+(-2)\times4$. \emph{Renvoi : M1.}
    \item \textbf{C} — L'eleve a change a tort le signe du premier produit et calcule $-3-8=-11$ au lieu de $3-8=-5$. \emph{Renvoi : M1.}
    \item \textbf{D} — L'eleve a oublie le signe negatif de la deuxieme composante et calcule $3+8=11$ au lieu de $3+(-8)=-5$. \emph{Renvoi : M1.}
  \end{itemize}
  \item \textbf{Q2}
  \begin{itemize}
    ...
    \item \textbf{D} — L'eleve a utilise a tort $\cos(\pi/4)=\sqrt{2}/2$ a la place de $\cos(\pi/3)=1/2$, obtenant $5\times4\times\sqrt{2}/2=10\sqrt{2}$. \emph{Renvoi : M1; R3.}
  \end{itemize}
  \item \textbf{Q3}
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{C} — L'eleve a interprete $\vec u\cdot\vec u$ comme la somme de deux exemplaires de la norme et additionne deux normes, d'ou $2\Vert\vec u\Vert$ au lieu de $\Vert\vec u\Vert^2$. \emph{Renvoi : M2.}
  ...
  \begin{itemize}
    ...
    \item \textbf{C} — L'eleve a transforme l'inegalite triangulaire $\Vert\vec u+\vec v\Vert\leqslant\Vert\vec u\Vert+\Vert\vec v\Vert$ en egalite et obtenu $3+4=7$ ; l'egalite n'est pas justifiee ici. \emph{Renvoi : M2.}
  ...
  \begin{itemize}
    ...
    \item \textbf{B} — Un produit scalaire negatif caracterise un angle obtus, pas un angle droit ; l'orthogonalite exige un produit scalaire nul. \emph{Renvoi : M3.}
  ...
  \begin{itemize}
    ...
    \item \textbf{C} — $\Vert\vec{u}\Vert = \Vert\vec{v}\Vert = \sqrt{13}$ : leurs normes sont egales, et non differentes. \emph{Renvoi : M3.}
    \item \textbf{D} — Des vecteurs de sens contraire sont colineaires avec un coefficient negatif ; ici leur determinant vaut $-13 \neq 0$, donc ils ne sont pas colineaires. \emph{Renvoi : M3.}
  \end{itemize}
  \item \textbf{Q9}
  \begin{itemize}
    \item \textbf{A} — L'eleve a utilise a tort $\tan(\theta)=1/\sqrt{3}$, ce qui conduit a $\theta=\pi/6$ ; les coordonnees de $(1,1)$ donnent en realite $\tan(\theta)=1$. \emph{Renvoi : M3; R3.}
    \item \textbf{C} — L'eleve a utilise a tort $\tan(\theta)=\sqrt{3}$, ce qui conduit a $\theta=\pi/3$ ; la pente du vecteur $(1,1)$ vaut $1$. \emph{Renvoi : M3; R3.}
  ...
  \begin{itemize}
    ...
    \item \textbf{D} — L'eleve a calcule le carre de la difference des deux longueurs, $(8-5)^2=9$, au lieu d'appliquer $a^2=b^2+c^2-2bc\cos(A)$. \emph{Renvoi : M5.}
  \end{itemize}
  \item \textbf{Q15}
  \begin{itemize}
    \item \textbf{A} — Signe $-$ au lieu de $+$. \emph{Renvoi : M5; R5.}
    \item \textbf{C} — L'eleve a conserve le terme $2bc$ et l'a pris pour $a^2$ ; lorsque $A=\pi/2$, le terme $2bc\cos(A)$ est nul, pas $a^2$. \emph{Renvoi : M5; R5.}
