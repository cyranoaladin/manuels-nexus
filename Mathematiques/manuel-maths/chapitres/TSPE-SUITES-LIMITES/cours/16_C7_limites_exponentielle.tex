...
\margeAppui{
\textbf{Résumé des limites fondamentales :} $\lim_{x \to +\infty} \mathrm{e}^x = +\infty$ ; $\lim_{x \to -\infty} \mathrm{e}^x = 0$ ; $\lim_{x \to +\infty} \dfrac{\mathrm{e}^x}{x} = +\infty$ ; $\lim_{x \to -\infty} x\,\mathrm{e}^x = 0$.
}
