% META: {"id": "1SPE-EXPONENTIELLE-FR-R2", "chapitre": "1SPE-EXPONENTIELLE", "type_objet": "remediation", "status": "generated", "prerequis_testes": ["R2"]}

%---------------------------------------------------------------
% FICHE DE REMISE A NIVEAU — R2 : Signe de la dérivée et variations
% Prerequis issu de 1SPE-DERIVATION-GLOBAL
%---------------------------------------------------------------

\subsection*{Rappel — Signe de la dérivée et sens de variation}
...
Pour dresser le tableau de variations de $f$, on etudie le signe de $f'$ sur son domaine de définition.
...
\begin{enumerate}
  \item Calculer $f'(x)$.
  \item Résoudre $f'(x) = 0$, puis déterminer le signe de $f'(x)$.
...
\commentaireMarge{$f'$ est négative avant $3$ (décroissance), positive après (croissance).}
...
\begin{enumerate}
  \item Calculer $g'(x)$.
  \item Résoudre $g'(x) = 0$, puis étudier le signe de $g'(x)$.
...
Soit $h(x) = \dfrac{x}{x^2 + 1}$ définie sur $\mathbb{R}$.
\begin{enumerate}
  \item Calculer $h'(x)$ a l'aide de la formule du quotient.
  \item Étudier le signe de $h'(x)$.
  \item En déduire le tableau de variations de $h$.
\end{enumerate}
\end{exercice}

\coupDePouce{1}{Le dénominateur $(x^2+1)^2$ est toujours positif. Le signe de $h'(x)$ depend uniquement du numérateur.}
...
\textbf{Question 2.} Le dénominateur $(x^2+1)^2 > 0$ pour tout $x$. Le signe de $h'(x)$ est celui de $1 - x^2 = (1-x)(1+x)$.
