...
\begin{enumerate}
  \item Étudier le sens de variation de $f$ sur $\mathbb{R}$ (on justifiera que $f'(x) > 0$ pour tout $x$).
  \item Montrer que l'équation $f(x) = 0$ admet une unique solution réelle $\alpha$.
  \item Justifier que $\alpha \in [0, 1]$.
  \item Écrire (sans l'executer) un algorithme Python de dichotomie qui renvoie un encadrement de $\alpha$ a $10^{-3}$ pres.
\end{enumerate}
\end{exercice}
