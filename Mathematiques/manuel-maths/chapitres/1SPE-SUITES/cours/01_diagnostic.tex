...
\begin{enumerate}
  ...
  \item Un capital de $2\,000$~€ subit deux évolutions successives : $+5$~\% puis $-5$~\%. Quel est le capital final ? Est-ce le même qu'après une évolution globale de $0$~\% ? Justifie.
...
\lstinputlisting[language=Python]{chapitres/1SPE-SUITES/python/1SPE-SUITES-DIAGNOSTIC-R5-SOMME.py}

  Quelle valeur est affichée ? Explique le rôle de la variable \texttt{s}.

  \item On exécute le programme suivant :

\lstinputlisting[language=Python]{chapitres/1SPE-SUITES/python/1SPE-SUITES-DIAGNOSTIC-R5-RECURRENCE.py}
