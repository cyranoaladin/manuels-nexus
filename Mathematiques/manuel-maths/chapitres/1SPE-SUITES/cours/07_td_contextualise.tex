% META: {"id": "1SPE-SUITES-COURS-07-TC", "chapitre": "1SPE-SUITES", "type_objet": "cours", "sous_type": "td_contextualise", "titre": "TD contextualise — Decroissance radioactive", "temps_gabarit": 7, "mode_creation": "ex_nihilo", "sources_inspiration": [], "status": "generated", "capacites_codes": ["C3", "C5", "C6", "C7"]}
% BEGIN-VERIFY
...
% assert abs(demi_vie_exacte - 56.94) < 0.01
...
% assert M(1) < M(0)
% # Quelques valeurs
% assert abs(M(10) - 88.54) < 0.01
% assert abs(M(100) - 29.60) < 0.01
% END-VERIFY
...
\begin{enumerate}
  ...
  \item Compléter le tableau de valeurs ci-dessous pour $n \in \{50, 55, 56, 57, 60\}$.
...
\textit{Détail :} $M_{56} = 100 \times 0{,}9879^{56} \approx 50{,}57$~g et $M_{57} = 100 \times 0{,}9879^{57} \approx 49{,}96$~g.

\textbf{Question 4.} Le plus petit entier $n$ tel que $M_n \leq 50$ est $n = 57$. La demi-vie est donc de $\mathbf{57}$ siècles, soit $\mathbf{5\,700}$ ans (la demi-vie réelle du carbone-14 est de $5\,730$ ans : excellente approximation).
...
\lstinputlisting[language=Python]{chapitres/1SPE-SUITES/python/1SPE-SUITES-TD-CONTEXTUALISE-LECTURE.py}
...
\lstinputlisting[language=Python]{chapitres/1SPE-SUITES/python/1SPE-SUITES-TD-CONTEXTUALISE-VALEURS.py}
...
\lstinputlisting[language=Python]{chapitres/1SPE-SUITES/python/1SPE-SUITES-TD-CONTEXTUALISE-DEMI-VIE.py}

Le programme affiche :
\begin{itemize}
  \item \texttt{Demi-vie : 57 siecles, soit 5700 ans}
  \item \texttt{Masse atteinte : 49.96 g}
\end{itemize}
...
\begin{itemize}
  ...
  \item Au rang $50$ : la perte est environ $54{,}41 \times 0{,}0121 \approx 0{,}66$~g.
\end{itemize}
