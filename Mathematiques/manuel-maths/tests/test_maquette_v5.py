...
if [ "$tool" = "lualatex" ]; then
  ...
  case "$NXV_FAKE_MODE:$*" in
    compare_diff_tab_01:*page-01.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_07:*page-07.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_08:*page-08.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_09:*page-09.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_10:*page-10.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_11:*page-11.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_12:*page-12.png*) printf '%s' '42' >&2; exit 1 ;;
    compare_diff_tab_15:*page-15.png*) printf '%s' '42' >&2; exit 1 ;;
...
def test_checker_cli_synthetic_exit_codes(tmp_path, monkeypatch):
    ...
    tab_reference_pages = (1, 7, 8, 9, 10, 11, 12, 15)
    for iteration in ("v5-it1", "v5-it2"):
        for page in tab_reference_pages:
    ...
    comparison_modes = tuple(
        f"compare_diff_tab_{page:02d}" for page in tab_reference_pages
    ...
    for mode in diagnostics_modes + comparison_modes + ("success",):
        monkeypatch.setenv("NXV_FAKE_MODE", mode)
        output_cases.append(
            subprocess.run(
                [
    ...
    invalid, question_marks, *acceptance_cases = output_cases
    diagnostics_cases = acceptance_cases[: len(diagnostics_modes)]
    comparison_cases = acceptance_cases[
        len(diagnostics_modes) : -1
    ]
    success = acceptance_cases[-1]
    ...
    for mode, rejected in zip(comparison_modes, comparison_cases):
        assert rejected.returncode == 1
        page = int(mode.rsplit("_", 1)[1])
        assert (
            f"page {page} différente de l'oracle it2: AE=42"
            in rejected.stderr
        )
...
def test_rubric_tab_dynamic_source_contract():
    ...
    assert r"\nxVOngletLength<20mm" not in tab_source
    assert (
        r"\setlength{\nxVOngletHalfLength}{\nxVOngletLength}"
        in tab_source
    )
...
\rubrique{Ouverture}
\noindent PAGE OUVERTURE IMPAIRE\vfill\newpage
\noindent PAGE OUVERTURE PAIRE\vfill\clearpage
...
""",
    ...
    lengths = {"Cours": [], "Ouverture": [], "Auto-évaluation": []}
    for page_number, label in enumerate(
        ...
        if label in ("Ouverture", "Auto-évaluation"):
        ...
        if label == "Ouverture":
            ...
            assert 16.0 * 300.0 / 25.4 < measured_length
            assert measured_length < 20.0 * 300.0 / 25.4
            assert abs(length - measured_length) <= half_point, (
                page_number,
                label,
                length,
                measured_length,
    ...
    assert abs(
        lengths["Ouverture"][0] - lengths["Ouverture"][1]
    ) <= half_point
    assert abs(
...
def test_validation_png_reference_hashes():
    checker = importlib.import_module("check_maquette_v5")
    expected = {
        1: "a3a5ea8b94c92028fad069d3ac11708bbfb51c883efbc2b6a011e69ff0592fd2",
        ...
        7: "0f091b2b8488f89de66884cc22b238450264791875568f8116e4e0bc65cf6280",
        8: "13ff6daab5f2d5fbd999af3e3f433f7bc92f2fc89f2adfd39f72bdf368e2b70f",
        9: "ccda0af7007a15ecf3b895b5cb60a2f658bcee78e7a725acad4d6590f54135c3",
        10: "4466330daf59618c2ab25947e244e08da3fcca1f446516af378f8884778e52d1",
        ...
        15: "11de1dad17368d27af7bf1ead77d3afad27188785806821bd01bdc5a1b1a9141",
    }
