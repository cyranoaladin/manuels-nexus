...
une boîte locale et mesure `max(16 mm, largeur + 6 mm)`. Les libellés courts du
spécimen restent sur la branche raster canonique de 16 mm ; les libellés longs,
dont `AUTO-ÉVALUATION`, réutilisent la dimension mesurée pour le rectangle et
le centre du texte sur les deux parités. Une fixture PDF teste la contenance par
corrélation entre BBox Poppler et composante colorée raster ; l'acceptation
complète conserve les anciens rendus p.11–12, consacre les nouveaux oracles
itération 2 et protège toutes les autres pages par hash/AE.

**Tech Stack:** LaTeX/LuaLaTeX, TikZ, KOMA-Script marks, Python 3.12, pytest, Pillow 12.3, Poppler (`pdftotext -bbox-layout`, `pdftoppm`), ImageMagick (`compare`, `identify`).

**Spec:** `docs/superpowers/specs/2026-07-20-dynamic-rubric-tab-length-design.md`

**Working directory de clôture:** toutes les commandes sont exécutées depuis
`Mathematiques/manuel-maths` dans le worktree isolé `1spe-bat-2026`, branche
`feature/1spe-bat-2026`.
...
- [x] **Step 1: figer les deux rendus défectueux avant toute recompilation.** Copier mécaniquement `validations/v5/page-11.png` et `page-12.png` vers `validations/v5-it1/`. Vérifier les SHA historiques directement contre le commit `f2aee3d` :
...
- [x] **Step 5: exécuter les deux tests rouges.**
...
- [x] **Step 1: déclarer les registres dédiés.** Ajouter une boîte, une longueur totale et une demi-longueur dont les noms sont spécifiques à l'onglet, par exemple :
...
- [x] **Step 2: composer et mesurer une fois dans un groupe local.** À l'intérieur de `\nxOngletRubrique`, après le test de suppression et avant TikZ :
...
contourner `\nxRubriquePage`. Pour préserver le contrat raster historique,
ramener à 16 mm les mesures strictement inférieures à 20 mm : cette branche
englobe les libellés courts du spécimen, tandis que `AUTO-ÉVALUATION` conserve
exactement `largeur + 6 mm`.
...
- [x] **Step 5: exécuter la fixture réelle.** Attendu : PASS sur les quatre pages et aucun `Overfull` dans la sortie LuaLaTeX.
...
- [x] **Step 1: écrire les attentes rouges du contrôleur.** Ajouter les constantes `HISTORICAL_TAB_PAGE_REFERENCE_SHA256` pour p.11–12 et `TAB_PAGE_REFERENCE_SHA256` pour les futurs oracles it2. Étendre les tests pour exiger : SHA historiques, SHA it2, `AE=0` p.11 et p.12, et rejet indépendant d'une différence sur chacun. Le faux environnement copie les quatre références p.11–12 et distingue `compare_diff_tab_11`, `compare_diff_tab_12` et `compare_diff_page_13`.
- [x] **Step 2: exécuter les tests synthétiques.**
...
- [x] **Step 3: recompiler la maquette complète sans modifier les hashes.**
...
- [x] **Step 4: prouver l'isolation avant consécration.** Calculer les SHA des quinze PNG et vérifier que seuls p.11–12 diffèrent de `f2aee3d`; vérifier que p.13 vaut toujours `2edeb64a24a83e38a88a0aefab83e54452eec3c9270cbeee3dc3afefb201af23`.
- [x] **Step 5: inspecter p.11 et p.12 à pleine résolution.** Utiliser `view_image` en détail original et des crops 100 %. Vérifier le libellé complet, le padding, le centrage, la symétrie, l'absence de contact avec le corps et la continuité de la couleur. Ne pas créer d'oracle si l'inspection échoue.
- [x] **Step 6: créer mécaniquement les oracles it2.** Copier les deux PNG inspectés vers `validations/v5-it2/`, calculer leurs SHA, puis renseigner `TAB_PAGE_REFERENCE_SHA256`. Remplacer uniquement les entrées 11 et 12 de `NON_DIAGNOSTICS_PAGE_SHA256`.
- [x] **Step 7: brancher les trois comparaisons actuelles.** Dans `accept_maquette`, vérifier d'abord les SHA it1 p.11–12, puis les SHA it2, puis `compare -metric AE = 0` entre oracles it2 et images générées p.11–12. Conserver sans changement les preuves et la comparaison p.13.
- [x] **Step 8: mettre à jour le faux outillage.** Copier les références réelles dans le `synthetic_root`; faire retourner `AE=42` uniquement pour la page désignée par le mode. Vérifier que les messages identifient p.11, p.12 ou p.13 sans ambiguïté.
- [x] **Step 9: exécuter les contrôles ciblés.**
...
- [x] **Step 1: mettre à jour le tableau AVANT/APRÈS.** Documenter p.11–12 : fond fixe 16 mm et texte blanc partiellement invisible → longueur automatique, police 6 pt, épaisseur 12 mm, padding minimal 3 mm, preuves géométriques et SHA des deux oracles.
- [x] **Step 2: exécuter le contrôleur public.**
...
- [x] **Step 3: exécuter les tests v5.**
...
- [x] **Step 4: exécuter les contrôles dépôt.**
...
Attendu : codes 0.
- [x] **Step 5: vérifier les invariants.** Le SHA QCM reste
...
- [x] **Step 8: conserver la branche isolée.** Worktree propre, branche
  `feature/1spe-bat-2026`, aucun merge/push/déploiement avant la validation
  technique de la baseline puis l'exécution du plan BAT.
