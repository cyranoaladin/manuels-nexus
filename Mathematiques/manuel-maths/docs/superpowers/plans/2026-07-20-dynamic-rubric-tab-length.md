...
- [x] **Step 4: écrire `test_rubric_tab_dynamic_fixture_pdf`.** Compiler une fixture de six pages : `Cours` p.1–2, `Ouverture` p.3–4, puis `Auto-évaluation` p.5–6, avec pages impaire et paire. Rendre à 300 dpi et extraire les six BBox. Pour chaque page, exiger inclusion du texte dans le rectangle et asymétrie longitudinale ≤0,5 mm. Exiger en plus :
  - onglet `COURS` = 16 mm avec tolérance raster ≤0,5 pt ;
  - onglet `OUVERTURE` strictement entre 16 et 20 mm, égal à
    `largeur + 6 mm` à ±0,5 pt et avec padding minimal ;
...
### Task 3: Oracles des huit pages d'onglets et contrôleur bout en bout

**Files:**
- Modify: `scripts/check_maquette_v5.py`
- Modify: `tests/test_maquette_v5.py`
- Modify: `validations/v5/page-01.png`, `page-07.png` à `page-12.png`,
  `page-15.png`
- Create: références it1/it2 des pages 1, 7–12 et 15
...
- [x] **Step 1: écrire les attentes rouges du contrôleur.** Ajouter les constantes `HISTORICAL_TAB_PAGE_REFERENCE_SHA256` et `TAB_PAGE_REFERENCE_SHA256` pour p.1,7–12,15. Étendre les tests pour exiger leurs SHA historiques, SHA it2, `AE=0` et le rejet indépendant d'une différence sur chaque page. Le faux environnement copie les seize références et distingue les huit modes `compare_diff_tab_XX`, plus `compare_diff_page_13`.
...
Attendu : RED, car les références it1/it2 des six pages intermédiaires et leur
branchement dans le contrôleur n'existent pas encore.
...
- [x] **Step 7: brancher les comparaisons actuelles.** Dans `accept_maquette`, vérifier d'abord les huit SHA it1, puis les huit SHA it2, puis `compare -metric AE = 0` entre chaque oracle it2 et image générée. Conserver sans changement les preuves et la comparaison p.13.
- [x] **Step 8: mettre à jour le faux outillage.** Copier les seize références réelles dans le `synthetic_root`; faire retourner `AE=42` uniquement pour la page désignée par le mode. Vérifier que les messages identifient séparément p.1,7–13,15.
...
Attendu : tous PASS ; AE p.1,7–13,15 = 0 ; 15 pages ; p.14 vide ; QCM canonique inchangé.
- [x] **Step 10: commit.**
...
- [x] **Step 1: mettre à jour le tableau AVANT/APRÈS.** Documenter p.1,7–12,15 : fond fixe 16 mm ou seuil parasite → longueur mathématique exacte, police 6 pt, épaisseur 12 mm, padding minimal 3 mm, preuves géométriques et SHA des huit couples it1/it2.
